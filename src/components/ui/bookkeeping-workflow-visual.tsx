...
import { ClipboardList, ArrowRightCircle, BarChart4, FolderSync } from "lucide-react"

export function BookkeepingWorkflowVisual() {
  ...
  return (
    <div className="relative w-full aspect-square max-w-[450px] mx-auto select-none pointer-events-none">
      ...
      {/* Node 1: Input / Raw Data (Top Left) */}
      <motion.div 
        ...
        <motion.div animate={floatAnimation(10, 0)} className="flex flex-col gap-3">
          <div className="flex items-center gap-2">
            <div className="p-1.5 rounded-md bg-[var(--color-bg-secondary)]">
              <ClipboardList className="w-5 h-5 text-[var(--color-slate)]" />
            </div>
          ...
          <div className="flex flex-col gap-1.5 pl-1">
            {[1, 2, 3].map((i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <div className="h-1.5 flex-1 bg-[var(--color-border)] rounded-full" />
              </div>
            ))}
          </div>
        </motion.div>
      ...
      >
        <motion.div animate={floatAnimation(13, 1)} className="flex flex-col items-center text-center gap-3">
          <div className="p-2.5 rounded-xl bg-white/10">
            <FolderSync className="w-6 h-6 text-[var(--color-accent-400)]" />
          </div>
          <div className="w-full flex justify-between gap-1 mt-1">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="h-1.5 flex-1 bg-white/30 rounded-full" />
            ))}
          </div>
        </motion.div>
      </motion.div>
      ...
      {/* Node 3: Output / Summaries (Bottom Right) */}
      <motion.div 
      ...
      >
        <motion.div animate={floatAnimation(11, 2)} className="flex flex-col gap-3">
          <div className="flex items-center gap-2">
            <BarChart4 className="w-5 h-5 text-[var(--color-primary-900)]" />
            <div className="h-2 flex-1 bg-[var(--color-text-secondary)] rounded-full" />
          </div>
          <div className="flex gap-2 h-8 items-end mt-1">
